...
#and checks for game status
class Engine:
    def __init__(self):
        """
        Initializes the game with a 4x4 board and adds two random blocks to start
        """
        self.size = 4
    ...
    def setBoard(self, boardString):
        """
        updates the board state from a string representation
        """
...
def mcts_strategy(game, num_simulations):
    """
     uses the Monte Carlo Tree Search algorithm to decide the best move
     It simulates possible moves and plays out random game scenarios to estimate the potential score
     like with one move score is high , and other is lower
     It Calculates the average score for each move and selects the move with the highest average score
    """
...
def get_game_container(html):
    """
    Extracts the game container’s HTML from the page source.
    first step
    """
...
if url_choice == '1':
    ...
    print("Invalid choice! Please enter either '1', '2'")
    exit()
